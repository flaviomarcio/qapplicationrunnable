...
void NotifyBase::onNotifyReceived(const QString &channel, const QVariant &payload){
    ...
    for(int methodIndex = 0; methodIndex < metaObject->methodCount(); ++methodIndex) {
        ...
        if(method.parameterCount()>2)
            ...
            QMetaType::Type pType=QMetaType::UnknownType;
            if(method.parameterCount()==1){
                pType=QMetaType::Type(method.parameterType(0));
            }
            else if(method.parameterCount()==2){
                pType=QMetaType::Type(method.parameterType(1));
            }
            ...
            if(pType==QMetaType::QVariantHash || pType==QMetaType::QVariantHash || pType==QMetaType::QVariantList || pType==QMetaType::QStringList){
